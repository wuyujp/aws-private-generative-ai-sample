...
def lambda_handler(event, context):
    ...
    response = generate_message(
        bedrock_agent_runtime, aiModel, knowledge_base_Id, user_prompt)
    response_text = response['output']['text']
    response_s3_uri_list = extract_s3_uris(response)
    response_s3_presigned_url_list = create_s3_presigned_url(
        response_s3_uri_list)
    response_dict = {"text": response_text,
                     "response_s3_presigned_url_list": response_s3_presigned_url_list}
